...
import com.akheparasu.tic_tac_toe.algorithms.*
    ...
    */
    ...
    LaunchedEffect(playerTurn) {
        if (!playerTurn && !player2.value) {
            ...
            val diff = Difficulty.Medium    // need this updated for difficulty
            runAITurn(grid, diff)           // calls new function using grid and diff
            count += 1              // count is not used,
                                    // if you want a function to check if someone won I made one

            //check if game complete, this does not work right now
